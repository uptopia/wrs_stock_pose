...
ros::Publisher top3_cloud_pub, stock_pub;
...
void yolov4_cloud_cb(const sensor_msgs::PointCloud2ConstPtr& organized_cloud_msg)
{
    int height = organized_cloud_msg->height;
    int width = organized_cloud_msg->width;
    int points = height * width;
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber sub_yolov4_cloud = nh.subscribe("/camera/depth_registered/points", 1, yolov4_cloud_cb);

    // top3_cloud_pub = nh.advertise<sensor_msgs::PointCloud2> ("/top3_cloud_pub", 1);
    stock_pub = nh.advertise<sensor_msgs::PointCloud2> ("/stock_pub", 1);
